...
# UHJ-Pi Raspberry Pi Setup Script - Generic Touch Version (180-degree rotation)
...
clear
echo "🎵 UHJ-Pi Raspberry Pi Setup Script - Generic Touch Version (180° Rotation) 🎵"
...
if [ -f "/home/$ACTUAL_USER/UHJ-Pi/start-gen.sh" ]; then
    # Add export right before sclang launch if not already present
    if ! grep -q "LIBINPUT_CALIBRATION_MATRIX" "/home/$ACTUAL_USER/UHJ-Pi/start-gen.sh"; then
        sed -i '/^sclang/i export LIBINPUT_CALIBRATION_MATRIX="-1 0 1 0 -1 1"' "/home/$ACTUAL_USER/UHJ-Pi/start-gen.sh"
        echo "✓ Updated start-gen.sh to export touch calibration matrix before launching sclang"
    fi
fi
