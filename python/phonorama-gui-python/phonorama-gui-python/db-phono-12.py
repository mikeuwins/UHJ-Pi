...
class PhonoControlGUI(tk.Tk):
    def __init__(self):
        ...
        self.monitor_button = ttk.Checkbutton(main_frame, text="Monitor Enable", variable=self.monitor_var, command=self.toggle_monitor)
        self.monitor_button.grid(row=1, column=0, sticky="w", pady=(10,0))

        self.headphone_var = tk.BooleanVar()
        self.headphone_button = ttk.Checkbutton(main_frame, text="Headphone Enable", variable=self.headphone_var, command=self.toggle_headphone)
        self.headphone_button.grid(row=2, column=0, sticky="w")

        self.input_buttons = {'line': self.line_button, 'mc': self.mc_button, 'mm': self.mm_button, 'mute': self.mute_button}
