...
def snap_to_nearest_input(value):
    db = -40 + (value / 127) * 52
    snapped_db = round(db * 2) / 2
    raw = round((snapped_db + 40) / 52 * 127)
    return raw

def snap_to_nearest_output(value):
    db = -55 + (value / 145) * 55
    snapped_db = round(db * 2) / 2
    raw = round((snapped_db + 55) / 55 * 145)
    return raw
...
class PhonoControlGUI(tk.Tk):
    ...
    def update_values(self, event=None):
        l = int(round(self.input_left_fader.get()))
        r = int(round(self.input_right_fader.get()))
        L = int(round(self.output_left_fader.get()))
        R = int(round(self.output_right_fader.get()))
        self.input_left_val.config(text=input_value_to_db(l))
        self.input_right_val.config(text=input_value_to_db(r))
        self.output_left_val.config(text=output_value_to_db(L))
        self.output_right_val.config(text=output_value_to_db(R))

    def on_input_left_change(self, event):
        val = snap_to_nearest_input(self.input_left_fader.get())
        self.input_left_fader.set(val)
        if self.input_link_var.get():
            self.input_right_fader.set(val)
            self.send_command(f"-l {val} -r {val}")
        else:
            self.send_command(f"-l {val}")
        self.update_values()

    def on_input_right_change(self, event):
        val = snap_to_nearest_input(self.input_right_fader.get())
        self.input_right_fader.set(val)
        if self.input_link_var.get():
            self.input_left_fader.set(val)
            self.send_command(f"-l {val} -r {val}")
    ...
    def on_output_left_change(self, event):
        val = snap_to_nearest_output(self.output_left_fader.get())
    ...
    def on_output_right_change(self, event):
        val = snap_to_nearest_output(self.output_right_fader.get())
        self.output_right_fader.set(val)
        if self.output_link_var.get():
            self.output_left_fader.set(val)
            self.send_command(f"-L {val} -R {val}")
