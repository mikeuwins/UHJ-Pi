...
class PhonoControlGUI(tk.Tk):
    def __init__(self):
        ...
        self.input_left_fader = ttk.Scale(input_left_frame, from_=127, to=0, orient="vertical", length=220, command=self.update_input_left_label)
        ...
        self.input_right_fader = ttk.Scale(input_right_frame, from_=127, to=0, orient="vertical", length=220, command=self.update_input_right_label)
        ...
        self.output_left_fader = ttk.Scale(output_left_frame, from_=145, to=0, orient="vertical", length=220, command=self.update_output_left_label)
        ...
        self.output_right_fader = ttk.Scale(output_right_frame, from_=145, to=0, orient="vertical", length=220, command=self.update_output_right_label)
    ...
    def update_input_left_label(self, val):
        self.input_left_val.config(text=input_value_to_db(float(val)))

    def update_input_right_label(self, val):
        self.input_right_val.config(text=input_value_to_db(float(val)))

    def update_output_left_label(self, val):
        self.output_left_val.config(text=output_value_to_db(float(val)))

    def update_output_right_label(self, val):
        self.output_right_val.config(text=output_value_to_db(float(val)))

    # Snap & set value on release
    def on_input_left_release(self, event):
        raw = snap_input_raw(self.input_left_fader.get())
        self.input_left_fader.set(raw)
        self.input_left_val.config(text=input_value_to_db(raw))

    def on_input_right_release(self, event):
        raw = snap_input_raw(self.input_right_fader.get())
    ...
    def on_output_left_release(self, event):
        raw = snap_output_raw(self.output_left_fader.get())
    ...
    def on_output_right_release(self, event):
        raw = snap_output_raw(self.output_right_fader.get())
