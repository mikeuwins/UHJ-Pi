...
class PhonoControlGUI(tk.Tk):
    def __init__(self):
        ...
        # Checkbuttons and reset
        right_frame = ttk.Frame(main_frame)
        right_frame.grid(row=0, column=1, sticky="n", padx=(20,0))

        self.monitor_var = tk.BooleanVar()
        self.monitor_button = ttk.Checkbutton(right_frame, text="Monitor Enable", variable=self.monitor_var, command=self.toggle_monitor)
        self.monitor_button.grid(row=0, column=0, pady=(0,10), sticky="w")

        self.headphone_var = tk.BooleanVar()
        self.headphone_button = ttk.Checkbutton(right_frame, text="Headphone Enable", variable=self.headphone_var, command=self.toggle_headphone)
        self.headphone_button.grid(row=1, column=0, pady=(0,10), sticky="w")

        self.input_link_var = tk.BooleanVar()
        self.input_link_button = ttk.Checkbutton(right_frame, text="Link Inputs", variable=self.input_link_var)
        self.input_link_button.grid(row=2, column=0, pady=(0,10), sticky="w")

        self.output_link_var = tk.BooleanVar()
        self.output_link_button = ttk.Checkbutton(right_frame, text="Link Outputs", variable=self.output_link_var)
        self.output_link_button.grid(row=3, column=0, pady=(0,10), sticky="w")

        self.reset_button = ttk.Button(right_frame, text="Reset", command=self.reset_values)
    ...
    def on_input_left_release(self, event):
        val = snap_input_raw(float(self.input_left_fader.get()))
    ...
    def on_input_right_release(self, event):
        val = snap_input_raw(float(self.input_right_fader.get()))
        if self.input_link_var.get():
            self.input_left_fader.set(val)
        self.input_right_fader.set(val)
    ...
    def on_output_left_release(self, event):
        val = snap_output_raw(float(self.output_left_fader.get()))
    ...
    def on_output_right_release(self, event):
        val = snap_output_raw(float(self.output_right_fader.get()))
        if self.output_link_var.get():
            self.output_left_fader.set(val)
        self.output_right_fader.set(val)
        self.send_command(f"-R {val}")
