...
# UHJ-Pi Touch Screen + HDMI Hybrid Installation Script
# Optimized for touch screen with X11/HDMI support and latest SuperCollider fixes
# Updated: August 2025
...
if [ -z "$ACTUAL_USER" ]; then
    echo "Error: Could not determine username. Please run with: sudo -E ./install-touch-hdmi-hybrid.sh"
    exit 1
fi

echo "UHJ-Pi Touch Screen + HDMI Hybrid Setup Script - Starting installation..."
...
# STEP 2: Configure for X11/HDMI operation
# Keep audio disabled for audio interface
if ! grep -q "dtparam=audio=off" /boot/firmware/config.txt; then
    echo "dtparam=audio=off" >> /boot/firmware/config.txt
fi
# Add dtoverlay for KMS support and disable audio (works with X11/HDMI)
...
# STEP 6: Configure SuperCollider Build - FIXED: SC_QT=ON for Qt support with X11
...
# STEP 15: Configure Qt platform for X11/HDMI operation
echo "Step 15: Configuring Qt platform for X11/HDMI operation..."
# Set Qt platform to xcb for X11 support (works with both touch and HDMI)
echo 'export QT_QPA_PLATFORM=xcb' >> /home/$ACTUAL_USER/.bashrc
echo 'export QT_QPA_PLATFORM=xcb' >> /home/$ACTUAL_USER/.profile
# Set X11 display variable for proper X11 operation
...
# STEP 17: Configure Blackbox theme for clean X11 interface
echo "Step 17: Configuring Blackbox theme for clean X11 interface..."
# Create custom Blackbox theme for minimal decorations
cat > /usr/share/blackbox/themes/NoDecorations.theme << 'EOF'
# NoDecorations theme for UHJ-Pi
# Minimal window decorations for professional appearance

# Window title bar
titlebar.font:		Sans-10:bold
titlebar.justify:	Center
titlebar.textColor:	#FFFFFF
titlebar.color:		#000000

# Window borders
window.clientColor:	#000000
window.labelColor:	#000000
window.buttonColor:	#000000
window.button.picColor:	#000000

# Menu appearance
menu.titleFont:		Sans-10:bold
menu.frameColor:		#000000
menu.titleColor:		#000000
menu.textColor:		#FFFFFF
menu.disabledColor:	#666666

# Toolbar
toolbar.color:		#000000
toolbar.textColor:	#FFFFFF
toolbar.labelColor:	#000000
EOF

# Configure Blackbox to use the theme
sudo -u $ACTUAL_USER mkdir -p /home/$ACTUAL_USER/.blackbox
cat > /home/$ACTUAL_USER/.blackboxrc << 'EOF'
# UHJ-Pi Blackbox Configuration
# Clean, minimal interface for professional audio applications

session.screen0.toolbar.placement:	TopLeft
session.screen0.toolbar.autoHide:	true
session.screen0.toolbar.widthPercent:	100
session.screen0.toolbar.isVisible:	true

session.screen0.workspaceCount:	1
session.screen0.defaultDeco:		NoDecorations

# Window placement
session.screen0.placement:		Smart
session.screen0.focusMode:		ClickToFocus
session.screen0.focusNewWindows:	true

# Style
session.screen0.toolbar.style:		NoDecorations
session.screen0.menu.style:		NoDecorations
EOF

echo "Blackbox theme configured for clean X11 interface"

echo "Installation completed successfully!"
echo ""
echo "🎉 UHJ-Pi Touch Screen + HDMI Hybrid Installation Complete! 🎉"
echo ""
echo "📱 Display: X11/HDMI compatible (works with both touch and HDMI)"
echo "🎨 Interface: Clean Blackbox theme with minimal decorations"
echo "⚡ Performance: Optimized SuperCollider build with latest fixes"
echo ""
...
echo "  ✅ Touch screen support with X11"
echo "  ✅ HDMI monitor support with X11"
echo "  ✅ Latest SuperCollider fixes"
echo "  ✅ Clean, professional interface"
echo "  ✅ All custom extensions and ATK support" 