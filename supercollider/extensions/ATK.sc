...
AtkKernelConv {
	*ar { |in, kernel, mul = 1, add = 0|
		...
		// Kernel structure: [WX_r, WX_i, YZ_r, YZ_i] per input channel
		// This is unique to PHJ - all other encoders have different shapes
		if((kernel.shape[0] == 4) && (kernel.shape[1] == 4), {
			// PHJ: Complex convolution with 4-lane interleaved format
			// WX lanes (0,1) produce W and X, YZ lanes (2,3) produce Y and Z
			var wxReal, wxImag, yzReal, yzImag;
			var w, x, y, z;
			
			// Process each input (L, R, T, Q) with its WX and YZ kernels
			// Pure convolution - no additions, just kernel multiplication
			...
			// W = sum of WX real parts, X = sum of WX imaginary parts
			// Y = sum of YZ real parts, Z = sum of YZ imaginary parts
			w = Mix.new(wxReal);
			x = Mix.new(wxImag);
			y = Mix.new(yzReal);
			z = Mix.new(yzImag);
