...
# UHJ-Pi Raspberry Pi Setup Script - USB Turntable Touch Version (180-degree rotation)
...
clear
echo "🎵 UHJ-Pi Raspberry Pi Setup Script - USB Turntable Touch Version (180° Rotation) 🎵"
...
if [ -f "/home/$ACTUAL_USER/UHJ-Pi/start-vin.sh" ]; then
    ...
    if ! grep -q "LIBINPUT_CALIBRATION_MATRIX" "/home/$ACTUAL_USER/UHJ-Pi/start-vin.sh"; then
        sed -i '/^sclang/i export LIBINPUT_CALIBRATION_MATRIX="-1 0 1 0 -1 1"' "/home/$ACTUAL_USER/UHJ-Pi/start-vin.sh"
        echo "✓ Updated start-vin.sh to export touch calibration matrix before launching sclang"
    fi
fi
