...
int enumerate_devices() {
    ...
    while (cur) {
        ...
        // Print serial number if known, otherwise print 'Unknown'
...
int main(int argc, char *argv[]) {
    ...
    // Load configuration (defaults if not present)
    ...
    while ((opt = getopt(argc, argv, "c:hiIdl:r:L:R:Mme")) != -1) {
        switch (opt) {
            ...
            case 'c':
                ...
                strncpy(selected_input, optarg, sizeof(selected_input) - 1);
                selected_input[sizeof(selected_input) - 1] = '\0';
            ...
            case 'l': cfg.input_l = clamp(atoi(optarg), 0, 127); break;
            case 'r': cfg.input_r = clamp(atoi(optarg), 0, 127); break;
            case 'L': cfg.output_l = clamp(atoi(optarg), 0, 145); break;
            case 'R': cfg.output_r = clamp(atoi(optarg), 0, 145); break;
    ...
    if (do_c || do_default) {
        gpio_data.ucGpioCtrl = 0x7F; // Always set this for GPIO
